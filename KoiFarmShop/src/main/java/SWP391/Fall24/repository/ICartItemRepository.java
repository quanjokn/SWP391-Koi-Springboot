...
public interface ICartItemRepository extends JpaRepository<CartItem, Integer> {
    ...
    @Query("select new SWP391.Fall24.pojo.CartItem(ci.fish.id, ci.cart, ci.fish, ci.quantity, ci.unitPrice, ci.totalPrice, ci.photo) from CartItem ci where ci.cart.id = :cartId ")
